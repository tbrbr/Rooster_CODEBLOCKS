...
class Pato {
    ...
    static int PauseMenu(RenderWindow* window, GameInfo & info) {
        ...
        Texture __t;
        __t.loadFromFile("..\\The_best_rooster_brazil2\\Patinho\\pause.jpg");
        ...
        Font font;
        font.loadFromFile("..\\The_best_rooster_brazil2\\fonts\\blops.ttf");
...
public:
    Pato(RenderWindow& window) {
        ...
        Color sc(r, g, b);
        for (int i = 0; i < 4; i++) {
            info.smira.aim[i].setFillColor(sc);
        }
        info.smira.px.setFillColor(sc);
        info.sensi = sensi;
        fclose(file);
