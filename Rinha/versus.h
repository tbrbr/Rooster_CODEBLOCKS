...
int versus(RenderWindow & window, Galo & p1, Galo & p2, RectangleShape & fundoluta, int __map)
{
	...
	time.restart();
	opacityTime.restart();
	Color ehfoda(255, 255, 255, 0);
	fundo.setFillColor(ehfoda);
	Color aaaaaaaaaaaaaaaaaaaaaaa(0, 0, 0, 255);
	fundoluta.setFillColor(aaaaaaaaaaaaaaaaaaaaaaa);
	...
	while (window.isOpen()) {
		...
		if (opacityTime.getElapsedTime().asMicroseconds() >= 4000) {
			if (animState == 1) {
                ...
                Color asd(255, 255, 255, fundoopacity);
				fundoluta.setFillColor(asd);
			}

			opacityTime.restart();
            Color namoralveyaaaaaaaaaaaaaaaaa(255, 255, 255, opacity);
			fundo.setFillColor(namoralveyaaaaaaaaaaaaaaaaa);
		}
